...
Below `lg:` (1024px) the composer wraps in a [Vaul](https://vaul.emilkowal.ski/) bottom sheet ([components/ui/drawer.tsx](components/ui/drawer.tsx) is the shadcn install). Two snap points: **peek (80px)** always visible above the system nav, and **expanded (280px)** for active composing — both pixel-based.

**Why a single expanded snap (not text-vs-voice):** earlier iterations had `TEXT_SNAP=0.55` and `VOICE_SNAP=0.9` so voice mode would auto-grow. In practice both modes' content fits comfortably in ~280px (voice's tallest state is the preview with ~150px of body + 80-ish px of sub-bar/handle/padding), and resizing the sheet on every mode toggle felt jarring — the user toggled modes, not the sheet. Single expanded snap = mode toggling is purely a content swap.

**Why 280px (pixel-based):** the snap is sized to the composer's worst-case content height (drag handle 18 + sub-bar 84 wrapped on narrow phones + body padding 24 + voice preview body 150 ≈ 276 → 280) rather than as a viewport fraction so we don't leave dead popover space and don't need to retune for different screen sizes. Long textareas grow inside the body via `field-sizing-content` and scroll through `overflow-y-auto` rather than expanding the sheet.

**Mobile keyboard handling:** Vaul's `repositionInputs` is left at its default (`true`) so the drawer lifts above the on-screen keyboard via `visualViewport`. An earlier 55vh + auto-lift combo had two real-device glitches: (1) the lift sometimes oversized the drawer to cover the viewport (whiting out the page underneath), and (2) on keyboard dismiss the drawer failed to restore to its snap, staying translated up where the keyboard had been. The much tighter 280px snap minimizes the surface area Vaul has to translate during the lift, which should avoid both glitches. If they recur in real-device testing, the fallback is `repositionInputs={false}` — the small snap keeps the keyboard overlay tolerable even without auto-lift (Android resizes the visible viewport on its own; iOS overlays the keyboard on top of the sheet's bottom).
...
**Why `h-full` on the drawer content:** Vaul's snap math expects the content element to fill its parent (the portal target / viewport). Constraining the height (e.g. `h-[280px]` to "match the largest snap") breaks Vaul's positioning math and the drawer ends up below the viewport. Over-drag past the expanded snap during the gesture is bounded by Vaul's release-snap (returns to nearest snap on release) plus the `dismissible={false}` floor at peek — not by capping the height.

**Mobile keyboard:** see "Mobile keyboard handling" above for the auto-lift trade-off and fallback. The `Textarea` primitive uses `text-base` (16px) on mobile and only switches to `text-sm` at `md:+`, so iOS won't auto-zoom on focus.
...
- `/rehearsals/[rehearsalId]` — Rehearsal workspace. Page header is a `RehearsalContextBar` (breadcrumb / title / role / meta); body is a two-column workspace with the stage-plate video + density timeline on the left and a thread (progress spine, pill filters + assignee/tag dropdowns, note list with tag + repeating chips) on the right. **Composer shape varies by viewport**: at `lg:+` it's a sticky card at the bottom of the right column; below `lg:` it's a peekable Vaul bottom sheet (80px peek + 280px expanded). The video pins to the top of the viewport on mobile via four contextual triggers — see "Mobile composer sheet" and "Contextual sticky video" under Rehearsal Workspace UI. Voice-note playback is video-synced. First-time note-authors see a 3-step `TipSequence` (timeline / composer / notes thread) once the video URL resolves — see "Onboarding tour" below.
