...
**Why 28vh (tuned tight):** real-device testing showed that the original 0.55 (55vh) snap left ~200px of dead popover background between the textarea and the iOS keyboard once Vaul lifted the sheet. Dropping to 28vh sits the lifted sheet flush above the keyboard with no wasted space, and is still tall enough to host the sub-bar + textarea + Post button (or the voice recorder's tallest state). Long textareas grow inside the body via `field-sizing-content` and scroll through `overflow-y-auto` rather than expanding the sheet.
...
**iOS keyboard:** Vaul's `repositionInputs` (default `true`) uses `visualViewport` to lift the drawer above the keyboard when an input focuses (iOS 16.4+). The `Textarea` primitive uses `text-base` (16px) on mobile and only switches to `text-sm` at `md:+`, so iOS won't auto-zoom on focus. The 28vh snap is tuned tight so the lifted sheet sits flush above the keyboard with no wasted background space.
