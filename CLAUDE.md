...
**iOS keyboard:** Vaul's `repositionInputs` (default `true`) uses `visualViewport` to lift the drawer above the keyboard when an input focuses (iOS 16.4+). The `Textarea` primitive uses `text-base` (16px) on mobile and only switches to `text-sm` at `md:+`, so iOS won't auto-zoom on focus. The 55vh snap leaves enough headroom (~40vh+) that the lifted sheet doesn't get its top clipped on typical phone viewports.

**Trade-off accepted:** drag-up past the expanded snap shows elastic over-drag during the gesture; Vaul snaps back to 0.55 on release. Could be tightened with custom drag interception but it's a recognized iOS bottom-sheet pattern, not a bug.
