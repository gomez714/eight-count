...
Below the `lg:` breakpoint the composer wraps in a [Vaul](https://vaul.emilkowal.ski/)-based bottom sheet with two snap points: an **80-pixel peek bar** (always visible above the system nav) and a **280-pixel expanded** state (sized to the composer's worst-case content height — voice preview with player + Save buttons — so there's no dead popover space). Both shells share the same internal `ComposerBody` component, so mode toggles, audience picker, tag picker, voice recorder, etc. are byte-identical to desktop — only the surrounding shell differs.
...
- **Tap interactions from peek**:
  ...
  - **"Tap to write/record…" / chevron** → expands to the single 280px snap.
- **Sheet behaviors**:
...
- **Mobile keyboard**: Vaul's `repositionInputs` is left at its default (`true`) — Vaul lifts the drawer above the on-screen keyboard via `visualViewport`. An earlier 55vh + auto-lift combo had two real-device glitches (the lifted drawer occasionally oversized to whiteout the page; on keyboard dismiss the drawer stayed translated up where the keyboard had been); the much tighter 280px snap minimizes the surface area Vaul has to translate, which should avoid both. If the glitches recur in real testing, set `repositionInputs={false}` on the `Drawer.Root` and rely on the small snap to keep the keyboard overlay tolerable.
