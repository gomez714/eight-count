...
- **Mobile keyboard**: Vaul's `repositionInputs` is **disabled** (`repositionInputs={false}`). Real-device testing surfaced two glitches with the auto-lift — the lifted drawer sometimes oversized to fill the viewport (whiting out the page), and on keyboard dismiss the drawer didn't restore to its snap. With auto-lift off, the keyboard simply overlays whatever is below it. The 55vh snap puts the textarea in the upper portion of the sheet so the input remains mostly visible above a typical mobile keyboard. A few pixels of overlap on the smallest phones is the accepted trade-off.
