...
- **Mobile keyboard**: Vaul's `repositionInputs` is **disabled** (`repositionInputs={false}`). Real-device testing surfaced two glitches with the auto-lift — the lifted drawer sometimes oversized to fill the viewport (whiting out the page), and on keyboard dismiss the drawer didn't restore to its snap. With auto-lift off, the OS handles the keyboard naturally — Android resizes the visible viewport so the sheet floats above the keyboard, iOS overlays the keyboard on top of whatever is below it. The 28vh snap is small enough that the page underneath the sheet stays comfortable when the keyboard appears.
