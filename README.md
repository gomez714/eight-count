...
- **Repeating clusters card** *(staff only)* appears above the rehearsals spine when any dancer in the project has 3+ unresolved notes with the same tag — one tinted row per cluster (avatar, name, tag chip, "N unresolved"). The "N unresolved" pill is **clickable** — expands the cluster's detail panel inline below the row (latest body, every instance's timestamp, link to the source rehearsal).
- **Drill board** *(staff only)* appears below the clusters card whenever the project has any open or in-progress notes — useful for instructors scanning who needs to drill what across the whole company. Ships with a **grouping toggle** in the section header:
  - **By dancer** (default) — one collapsible card per dancer → tag sections inside. Answers "is Iris OK?"
  - **By tag** — one collapsible card per tag → dancers listed inside with their items. Answers "who needs to be in the timing sectional?" Choice persists in the URL via `?groupBy=tag`. Each mode remembers its own expand/collapse state, so flipping back finds your previously-open cards still open.
  - Within either mode, rows sort by priority within their tag bucket: items in a repeating cluster first, then oldest unresolved, then newest rehearsal. Repeating chips are expandable here too.
  - In By-dancer mode, the viewer's own row defaults expanded if they're a recipient; otherwise the dancer with the most clusters expands by default. Dancers don't see this card or the clusters card on the project page; their personal drill list lives at `/my-notes` (Drill view) instead, scoped to just their own notes.
...
- **"Drill from this rehearsal" button** appears in the context bar's actions slot when the viewer has at least one active assignment in this rehearsal — one click deep-links to `/my-notes?view=drill&rehearsal=<id>` with the personal drill inbox pre-scoped to just this session. Visible to dancers and staff alike (anyone with work to drill sees it). Hidden when the viewer has nothing active in this rehearsal — no empty-state surprises.
...
- **Expandable cluster details**: in drill surfaces (both `/my-notes?view=drill` and the project page) the repeating chip is interactive — click it to reveal an inline panel showing every instance's timestamp (clickable to jump to its rehearsal), the most-recent note's body (text or voice transcript), and a "View latest note in {rehearsalTitle}" link. Turns the flag from decorative into actionable. On mobile only one panel is open at a time; on desktop multiple panels accumulate for side-by-side comparison. Panels are excluded from print output (same info is in the underlying drill row list, no need to duplicate ink).
...
- **Right column:**
  - **Notes / Discussions tab switcher** at the top — segmented control with per-tab counts. Notes (corrections that someone owes follow-through on) and discussions (open-ended creative questions, see "Discussions" below) live side-by-side; only one list is mounted at a time. Drafts in either composer survive a tab switch. Tab switching is blocked mid-recording so the user can't lose a take.
  - **Progress spine** *(notes tab only)* — aggregate per-recipient progress across all notes: an "X / Y addressed-or-resolved" headline, open and in-progress counts, and a four-segment stacked bar broken down by `OPEN / IN_PROGRESS / ADDRESSED / RESOLVED`.
  - **Filter pills** *(notes tab)* — single-select pills: `All / Open / In progress / Addressed / Resolved / Unassigned / Voice / @ me`, plus a separate dropdown to filter by an arbitrary assignee. Pills show a precomputed count when inactive; a "Showing X of Y" indicator reflects the result set.
  - **Note thread** *(notes tab)* — each note row has a fixed timestamp rail (clickable to jump the video), a coral or teal accent stripe distinguishing voice from text, the author + audience chips, the body (text or voice waveform), a dashed-divider "Assigned" row of avatar + name + status-dot + status-label chips per recipient, and a collapsible **conversation chip** (`💬 N replies · 👍 a · 🙋 q · ❤️ e`) that expands to the note's conversation — see "Note conversations" below.
  - **Discussion list** *(discussions tab)* — see "Discussions" below.
  - **Composer** — context-aware. In the notes tab: a sub-bar with the Text / Voice mode toggle, a "To" audience picker (popover: full-cast quick-pick, groups, individuals, removable chips), an optional **Tag** picker, and a locked-timestamp pill that re-captures the current playhead when clicked. Body morphs between a 2-row textarea + Post button (text mode) and the voice recorder (voice mode). In the discussions tab: a lighter composer — no audience picker (discussions are team-wide), no tag picker (deferred), but a Pin / PinOff anchor toggle that swaps between video-anchored and "Rehearsal-wide" (no timestamp) modes. On **desktop** (≥1024px) the composer is a sticky card at the bottom of the right column. On **mobile** (<1024px) the same composer body lives inside a peekable bottom sheet — see "Mobile composer sheet" below.

#### Mobile composer sheet
Below the `lg:` breakpoint the composer wraps in a [Vaul](https://vaul.emilkowal.ski/)-based bottom sheet with **three snap points**: an **80-pixel peek bar** (always visible above the system nav), a **220-pixel expanded** state (general composing — voice preview, idle text), and a **340-pixel writing** state (auto-activated when the user focuses the text-mode textarea or opens the inline audience picker). All three are content-sized — no viewport fractions, no dead zones above the keyboard. Both desktop and mobile shells share the same internal `ComposerBody` component, so mode toggles, audience picker, tag picker, voice recorder, etc. are byte-identical to desktop — only the surrounding shell differs.
...
- **Writing mode** (340px snap): when the user focuses the textarea, the sheet promotes to the writing snap, the textarea bumps to `min-h-[180px]`, and the **video + timeline hide on mobile** to reclaim vertical space for the keyboard. The captured timestamp pill in the composer header stays visible as the load-bearing reference — the user doesn't need the live video frame while typing. Doesn't auto-demote on blur (user might be tapping the tag/audience picker between keystrokes); a successful send auto-collapses to peek instead. Voice mode never enters writing mode (no textarea, no focus event).
- **Inline audience picker (mobile)**: on phones the audience picker renders as the body of the sheet (replacing the textarea/recorder) instead of as a popover overlay. The trigger is a plain toggle button; tapping it promotes to the writing snap, hides video + timeline, and renders a "Pick audience" panel with the same group/individual toggles plus a **Done** button to return to composing. This eliminates the popover-over-sheet-over-keyboard stacking that the desktop popover would create on small viewports — the search input is in the sheet directly, the keyboard opens against a comfortable 340px surface, no collision math.
- **Tap interactions from peek**:
  ...
  - **Audience chip** → opens the picker (inline on mobile, popover on desktop). On mobile, the workspace's audience-open handler promotes the sheet to the writing snap and the inline panel renders.
  - **Mode toggle** → swaps mode but doesn't expand. Suppressed during recording (would unmount the recorder mid-take). If audience was open, it closes (the user has explicitly switched composing surfaces).
  - **"Tap to write/record…" / chevron** → expands to the 220px snap. Tapping into the textarea after that promotes to 340px automatically.
- **Sheet behaviors**:
  ...
  - **Vaul focus-trap escape**: Vaul doesn't pass `modal={false}` through to Radix `DialogPrimitive.Root`, so Radix's `FocusScope` would otherwise trap focus inside the drawer (stealing it back from comment-composer textareas outside the sheet). A capture-phase `focusin` listener on the document stops propagation before Radix's bubble-phase listener sees it, so focus stays wherever the user actually tapped. Without this, tapping a comment input below the sheet would silently bounce focus back to the note composer.
...
- **Mobile keyboard**: `repositionInputs={false}` (explicit override of Vaul's default `true`). An earlier auto-lift combo had two real-device glitches (oversized drawer whiting out the page; drawer staying translated up after keyboard dismiss). The 340px writing snap accounts for the keyboard's footprint directly, so Vaul isn't trying to lift anything — the page resizes around the keyboard naturally on Android, and the writing snap stays usable on iOS even with the keyboard overlaying the bottom. The `Textarea` primitive uses `text-base` (16px) on mobile so iOS won't auto-zoom on focus.
...
### Note conversations (comments + reactions)
Every note carries an optional **conversational layer** — a flat thread of text replies, a fixed set of reactions, and per-viewer unread tracking — so dancers can ask clarifying questions and authors can follow up without leaving the app. Status (Open / In progress / Addressed / Resolved) answers "where is this note in your work?"; comments + reactions answer "what's the back-and-forth about it?"

- **Where threads appear**: every note row across the rehearsal workspace, `/my-notes`, and `/notes-by-me` has a collapsible thread chip at the bottom showing `💬 N replies · 👍 a · 🙋 q · ❤️ e`. Clicking it expands the thread in place — read replies, react, and post your own from the same surface, no separate page. Hidden in drill view (`/my-notes?view=drill`) since drill mode is print-focused.
- **Reactions**: a fixed set of three — **👍 Acknowledge** ("got it, I've seen this"), **🙋 Question** ("I have a question about this note" — doubles as the "dancer questions" signal), **❤️ Encourage** ("sending a little support"). One of each kind per user per note; click again to remove.
- **Replies**: text-only, up to 2000 characters, `Cmd/Ctrl + Enter` to send. Authors can edit (small `· edited` indicator after the timestamp) or soft-delete their own comments — deletion leaves a muted "Comment deleted" tombstone so thread continuity is preserved. Voice replies, reactions on individual comments, and nested replies are explicit non-goals for v1.
- **Visibility**: threads are **public within the team** — any team member can read and post. A reply to a note targeted at "Front line" is visible to all of Front line plus the author. No DM-style private back-channel; if you need that, send a separate note.
- **Mobile: one thread at a time**: on phones, opening a thread auto-collapses any other open thread. Comment drafts are preserved per-thread so context-switching doesn't lose in-progress text. Desktop allows multiple threads expanded simultaneously for side-by-side comparison.
- **Dashboard "new replies"**: when you have unread comments waiting — on a note you authored or were assigned, *or* on any discussion in a team you belong to — a compact `💬 N new` indicator surfaces on `/dashboard`, both as a meta chip in the welcome band and as a line under the work tiles. Notes use **engagement-scoped** counting (only when you're directly involved); discussions use **membership-scoped** counting (anywhere you can see). Counts refresh on dashboard navigation; the per-thread unread dot clears the moment you expand that thread.

### Discussions
Discussions are the conversational counterpart to notes. Where a **note** is an actionable correction with assignment + status + follow-through ("hit the 1-2 cleaner here"), a **discussion** is an open-ended creative or process question ("what quality should we engage in this section?", "would this work better another way?"). Discussions have **no assignment, no status, no follow-through** — author + thread only — and are deliberately **open to everyone**, dancers included. Notes flow downward from authority; discussions flow horizontally across the team.

- **Scope**: every discussion belongs to a **project**. Optionally it's also anchored to a **rehearsal** (rolls up to the project page from that session), and optionally to a **moment** on that rehearsal's video. The three shapes that result — project-wide, rehearsal-wide (no timestamp), and video-anchored — are all expressible. Range timestamps are permitted by the schema but the v1 UI only exposes single-timestamp + un-anchored modes.
- **Voice support, day one**: discussions can be text or voice, using the same recorder + Deepgram transcription as voice notes. Voice always requires a rehearsal anchor (the audio is tied to a specific rehearsal's recording). Project-level voice is deliberately not supported in v1 — text only at the project scope.
- **Anyone authors**: all four roles can create, edit, and delete their own discussions. The audio-upload route applies a `?purpose=discussion` flag that bypasses the staff role gate so dancers can record voice discussions; the transcript-retry route is loosened to "author OR staff" so dancers can retry their own failed transcripts.
- **Where discussions appear**:
  - **Rehearsal workspace** (`/rehearsals/[id]`) — behind a Notes / Discussions tab switcher in the right column. The discussion timeline marker uses an indigo accent (`--discussion-accent`) so it's visually distinct from the teal/coral note markers; un-anchored discussions appear in the list but not on the timeline.
...
- **Drill view** strips the page down to a tag-grouped read-only checklist (Timing → Spacing → … → Other), with a "Recurring drills" header at the top surfacing any clusters as **expandable chips** (click to see the cluster's timestamps + latest body + source-note link). For dancers in two or more active projects, the view auto-narrows to the busiest project and shows a "Showing N notes from {project} — see all projects" header so they always know what show they're drilling. Each row carries the project name as a small chip when there's ambiguity. **Within each tag section, rows sort by priority**: items in a repeating cluster first, then oldest unresolved, then newest rehearsal — the top of every tag is the most urgent thing to drill. **Voice notes appear inline as their transcript text** when transcription has succeeded — you can read what to drill on without playing audio, which is the load-bearing piece for printing or studying away from the device. A **Print** button calls the browser's print dialog; a CSS print stylesheet hides chrome (and the expanded cluster panels — they'd just duplicate the row list) and reformats the page for paper / "Save as PDF".
- **Rehearsal-scoped drill** (`?rehearsal=<id>`): coming from a rehearsal workspace via the "Drill from this rehearsal" button narrows the drill view to that one rehearsal's notes. A "Showing N notes from {rehearsal title} — see all rehearsals" header sits above the buckets; clearing it drops the URL param and returns to the unscoped view. The rehearsal filter survives toggling between Inbox and Drill so a bookmark to `?view=drill&rehearsal=<id>` (or back-and-forth navigation) keeps the scope intact until you explicitly clear it.
...
- `DATABASE_URL` — PostgreSQL connection string. Used by both the running app and `prisma migrate`. The schema doesn't declare a separate `directUrl`, so a single URL covers both. For prod, prefer the non-pooled URL if your provider offers one (Neon, Supabase) — pooled connections through PgBouncer can occasionally trip during DDL, though the migrations in this repo are small enough that the pooler URL works in practice too.
