...
// Resumable-upload counterpart to /upload-url for audio. The `?purpose=`
// query param keeps the same semantics as on /upload-url — default is
// staff-only (voice notes); `?purpose=discussion` opens the route to any
// team member (voice discussions). See "Voice Note Recording Flow" in
// CLAUDE.md.
export async function POST(
  request: NextRequest,
  context: { params: Promise<{ rehearsalId: string }> }
): Promise<NextResponse<AudioUploadSessionResponse>> {
  try {
    ...
    if (!callerMembership) {
      return apiError(403, "FORBIDDEN", "You are not a member of this team.");
    }
    ...
    const body = (await request.json()) as Partial<AudioUploadSessionRequest>;
    ...
    console.error("[upload] failed to create audio upload session:", error);
    return apiError(
      500,
      "AUDIO_UPLOAD_SESSION_CREATE_FAILED",
      error instanceof Error
        ? error.message
        : "Failed to create audio upload session"
    );
  }
}
